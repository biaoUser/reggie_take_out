...
import com.biao.mapper.OrderMapper;
...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Orders> implements OrderService {
    ...
    public void submit(Orders orders) {
        ...
        List<ShoppingCart> carts = shoppingCartService.list(shoppingCartQueryWrapper);
        if (carts.size()==0||carts==null){
            throw new CustoException("购物车为空不可下单");
        }
        //查询用户数据
        ...
        if (addressBook==null){
            throw  new CustoException("用户地址有误，不可下单");
        }
        //向订单表插入一条数据
        //①准备订单号
